...
# if d(a) = b, and d(b) = a, then a and b are an amicable pair, and
...
for i in range (1, 10000):
	divisorArray = getDivisors(i)
	# this array includes the number itself
	divisorSum[i] = sum(divisorArray) - i
...
for i in range (1, 10000):
	...
	if potentialPartner in divisorSum:	# then we have calculated d(b) already
		...
		potentialPartnerDivisorSum = sum(getDivisors(potentialPartner)) - potentialPartner
